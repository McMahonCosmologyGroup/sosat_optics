...
def aperature_fields(P_rx, tele_geo, plot, col):
    ...
    for ii in range(n_pts):
        ...
        x_rx_temp, y_rx_temp, z_rx_temp = tele_into_m3b(
            x_0, y_0, z_0)  # P_rx temp
        ...
        tan_rx_m3b_t[1] = tan_rx_m3b[1] * \
            np.cos(th2_l3) - tan_rx_m3b[2] * np.sin(th2_l3)
        tan_rx_m3b_t[2] = tan_rx_m3b[1] * \
            np.sin(th2_l3) + tan_rx_m3b[2] * np.cos(th2_l3)

        tan_og_t = np.zeros(3)
        tan_og_t[0] = tan_og_si[0]
        tan_og_t[1] = tan_og_si[1] * \
            np.cos(th2_l3) - tan_og_si[2] * np.sin(th2_l3)
        tan_og_t[2] = tan_og_si[1] * \
            np.sin(th2_l3) + tan_og_si[2] * np.cos(th2_l3)
        ...
        norm_temp = np.array([-norm[0], -norm[1], 1])
        N_hat = norm_temp / np.sqrt(sum(norm_temp ** 2))
        vec_m3b_m3a = np.array([x_m3a_temp,
                                y_m3a_temp,
                                z_m3a_temp]) - np.array([x_m3b_temp,
                                                         y_m3b_temp,
                                                         z_m3b_temp])
        ...
        tan_m3b_m3a_t[1] = tan_m3b_m3a[1] * \
            np.cos(th1_l3) - tan_m3b_m3a[2] * np.sin(th1_l3)
        tan_m3b_m3a_t[2] = tan_m3b_m3a[1] * \
            np.sin(th1_l3) + tan_m3b_m3a[2] * np.cos(th1_l3)

        tan_og_t[0] = tan_og_vac[0]
        tan_og_t[1] = tan_og_vac[1] * \
            np.cos(th1_l3) - tan_og_vac[2] * np.sin(th1_l3)
        tan_og_t[2] = tan_og_vac[1] * \
            np.sin(th1_l3) + tan_og_vac[2] * np.cos(th1_l3)
        ...
        norm_temp = np.array([-norm[0], -norm[1], 1])
        N_hat = norm_temp / np.sqrt(sum(norm_temp ** 2))
        vec_m3a_m2b = np.array([x_m2b_temp,
                                y_m2b_temp,
                                z_m2b_temp]) - np.array([x_m3a_temp,
                                                         y_m3a_temp,
                                                         z_m3a_temp])
        ...
        tan_m3a_m2b_t[1] = tan_m3a_m2b[1] * \
            np.cos(th1_l2) - tan_m3a_m2b[2] * np.sin(th1_l2)
        tan_m3a_m2b_t[2] = tan_m3a_m2b[1] * \
            np.sin(th1_l2) + tan_m3a_m2b[2] * np.cos(th1_l2)

        tan_og_t[0] = tan_og_si[0]
        tan_og_t[1] = tan_og_si[1] * \
            np.cos(th1_l2) - tan_og_si[2] * np.sin(th1_l2)
        tan_og_t[2] = tan_og_si[1] * \
            np.sin(th1_l2) + tan_og_si[2] * np.cos(th1_l2)
        ...
        norm_temp = np.array([-norm[0], -norm[1], 1])
        N_hat = norm_temp / np.sqrt(sum(norm_temp ** 2))
        vec_m2b_m2a = np.array([x_m2a_temp,
                                y_m2a_temp,
                                z_m2a_temp]) - np.array([x_m2b_temp,
                                                         y_m2b_temp,
                                                         z_m2b_temp])
        ...
        tan_m2b_m2a_t[1] = tan_m2b_m2a[1] * \
            np.cos(th2_l2) - tan_m2b_m2a[2] * np.sin(th2_l2)
        tan_m2b_m2a_t[2] = tan_m2b_m2a[1] * \
            np.sin(th2_l2) + tan_m2b_m2a[2] * np.cos(th2_l2)

        tan_og_t[0] = tan_og_vac[0]
        tan_og_t[1] = tan_og_vac[1] * \
            np.cos(th2_l2) - tan_og_vac[2] * np.sin(th2_l2)
        tan_og_t[2] = tan_og_vac[1] * \
            np.sin(th2_l2) + tan_og_vac[2] * np.cos(th2_l2)
        ...
        norm_temp = np.array([-norm[0], -norm[1], 1])
        N_hat = norm_temp / np.sqrt(sum(norm_temp ** 2))
        vec_m2a_m1b = np.array([x_m1b_temp,
                                y_m1b_temp,
                                z_m1b_temp]) - np.array([x_m2a_temp,
                                                         y_m2a_temp,
                                                         z_m2a_temp])
        ...
        tan_m2a_m1b_t[1] = tan_m2a_m1b[1] * \
            np.cos(th2_l1) - tan_m2a_m1b[2] * np.sin(th2_l1)
        tan_m2a_m1b_t[2] = tan_m2a_m1b[1] * \
            np.sin(th2_l1) + tan_m2a_m1b[2] * np.cos(th2_l1)

        tan_og_t[0] = tan_og_si[0]
        tan_og_t[1] = tan_og_si[1] * \
            np.cos(th2_l1) - tan_og_si[2] * np.sin(th2_l1)
        tan_og_t[2] = tan_og_si[1] * \
            np.sin(th2_l1) + tan_og_si[2] * np.cos(th2_l1)
        ##################################################
        ...
        norm_temp = np.array([-norm[0], -norm[1], 1])
        N_hat = norm_temp / np.sqrt(sum(norm_temp ** 2))
        vec_m1b_m1a = np.array([x_m1a_temp,
                                y_m1a_temp,
                                z_m1a_temp]) - np.array([x_m1b_temp,
                                                         y_m1b_temp,
                                                         z_m1b_temp])
        ...
        tan_m1b_m1a_t[1] = tan_m1b_m1a[1] * \
            np.cos(th1_l1) - tan_m1b_m1a[2] * np.sin(th1_l1)
        tan_m1b_m1a_t[2] = tan_m1b_m1a[1] * \
            np.sin(th1_l1) + tan_m1b_m1a[2] * np.cos(th1_l1)

        tan_og_t[0] = tan_og_vac[0]
        tan_og_t[1] = tan_og_vac[1] * \
            np.cos(th1_l1) - tan_og_vac[2] * np.sin(th1_l1)
        tan_og_t[2] = tan_og_vac[1] * \
            np.sin(th1_l1) + tan_og_vac[2] * np.cos(th1_l1)
        ...
        if plot == 1:
            if np.mod(ii, 51) == 0:
                ...
                plt.plot([y_m1b, y_m1a], [z_m1b, z_m1a],
                         "-", color=col, alpha=alph)
                plt.plot([y_m1a, pos_ap[1]], [z_m1a, pos_ap[2]],
                         "-", color=col, alpha=alph)
...
def rx_to_lyot(P_rx, tele_geo, plot, col):
    ...
    for ii in range(n_pts):
        ...
        if ( (pos_lyot[0]**2 + pos_lyot[2]**2) <= 210**2) and ((x_m2a ** 2 + z_m2a ** 2) <= (210**2)):
